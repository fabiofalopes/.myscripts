...
**Remaining**: S6/C4 stale doc refs **cleaned 2026-09-03** — all `~/.myscripts/` paths repointed to current locations (extracted projects → hub/, repo files → `~/projetos/hub/.myscripts/`); references to permanently deleted files (fabric-vision scripts, my-workflow) left as historical record. G3.7 (or-bench live test) needs an API key. G4–G6 (low-priority greenfields) work fine as scripts. See `HUMAN.md` for the decision record and `temp/prompt_2026-06-29_0157_stabilization-ops.md` for the runbook.
