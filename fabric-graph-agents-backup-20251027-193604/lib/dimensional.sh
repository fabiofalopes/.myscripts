...
extract_dimensions() {
    ...
    while [ "$action" = "refine" ] && [ $attempt -lt $max_retries ]; do
        ...
        if [ $attempt -eq 1 ]; then
            extraction=$(echo "$input" | fabric -p dimension_extractor_ultra)
        else
            # Include refinement guidance
            local suggestions=$(echo "$validation" | jq -r '.suggestions[]' 2>/dev/null || echo "")
            local refined_input="$input\n\nREFINEMENT GUIDANCE:\n$suggestions"
            extraction=$(echo -e "$refined_input" | fabric -p dimension_extractor_ultra)
        fi
        
        # Validate extraction
        validation=$(echo "$extraction" | fabric -p validate_extraction)
