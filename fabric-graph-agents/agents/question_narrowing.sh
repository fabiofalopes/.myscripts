...
main() {
    ...
    if [ -z "$input" ]; then
        ...
        echo "  FABRIC_MODEL=openai/gpt-4o $0 'How secure am I?'"
        exit 1
    fi
    
    if [ -n "$FABRIC_MODEL" ]; then
        log "📡 Using model: $FABRIC_MODEL"
    fi
...
EOF
    ...
    cat "$temp_input" | fabric_call -p extract_questions > "$RUN_DIR/output/node-1.md" 2>&1 || {
    ...
    cat "$RUN_DIR/output/node-1.md" | fabric_call -p improve_prompt > "$RUN_DIR/output/node-2.md" 2>&1 || {
    ...
    cat "$RUN_DIR/output/node-2.md" | fabric_call -p ask_secure_by_design_questions > "$RUN_DIR/output/node-3.md" 2>&1 || {
    ...
    cat "$RUN_DIR/output/final.md" | fabric_call -p extract_questions > "$RUN_DIR/output/follow-up-questions.md" 2>&1 || {
