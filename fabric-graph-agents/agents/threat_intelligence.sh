...
EOF
    ...
    cat "$temp_input" | fabric-ai -p analyze_threat_report > "$RUN_DIR/output/node-1.md" 2>&1 || {
    ...
    cat "$RUN_DIR/output/node-1.md" | fabric-ai -p search_query_generator > "$RUN_DIR/output/node-2.md" 2>&1 || {
    ...
    cat "$RUN_DIR/output/node-2.md" | fabric-ai -p deep_search_optimizer > "$RUN_DIR/output/node-3.md" 2>&1 || {
    ...
    cat "$RUN_DIR/output/node-3.md" | fabric-ai -p search_refiner > "$RUN_DIR/output/node-4.md" 2>&1 || {
