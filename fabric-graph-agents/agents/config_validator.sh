...
EOF
    ...
    cat "$temp_input" | fabric-ai -p ask_secure_by_design_questions > "$RUN_DIR/output/node-1.md" 2>&1 || {
    ...
    log "   → Analyzing risks..."
    cat "$RUN_DIR/output/node-1.md" | fabric-ai -p analyze_risk > "$RUN_DIR/output/node-2.md" 2>&1 || {
    ...
    cat "$RUN_DIR/output/node-2.md" | fabric-ai -p create_report_finding > "$RUN_DIR/output/node-3.md" 2>&1 || {
    ...
    cat "$RUN_DIR/output/final.md" | fabric-ai -p extract_recommendations > "$RUN_DIR/output/remediation-checklist.md" 2>&1 || {
